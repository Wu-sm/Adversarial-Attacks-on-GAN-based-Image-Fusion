...
from stylefusion.sf_stylegan2_hook import SFGenerator_hook
...
class StyleFusionSimple:
    def __init__(self, stylegan_type, stylegan_weights, fusion_nets_weights,device,GAN=None):
        ...
        # else:
        #     self.original_net.load_state_dict(GAN.state_dict(), strict=True)

        ##############################HOOK###############
        self.original_net = SFGenerator_hook(self.stylegan_size, 512, 8, GAN, stylegan_weights)
        ...
        for key in keys:
            self.sf_hierarchy.nodes[key].load_fusion_net('/home/sh/pycharm_workspace/StyleFusion-main/'+fusion_nets_paths[key], self.device)
    ...
    def s_to_image(self, s):
        ...
        img, festures, _ = self.original_net([torch.zeros(1, 512, device=self.device)],
                                             randomize_noise=False, style_vector=s)
        return img, festures
