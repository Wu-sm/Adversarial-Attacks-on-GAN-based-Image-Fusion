...
def benign_fusion_spatial(benign_savedir, all_latents, drawer, args,batch_idx):
    ...
    if args.save_img:
        vutils.save_image((inputs + 1) / 2, os.path.join(benign_savedir, 'spatial_org_inputs_%d.jpg' % batch_idx))
...
def benign_fusion_arithmetic(benign_savedir, all_latents, drawer, args,batch_idx):
    ...
    if args.save_img:
        vutils.save_image((inputs + 1) / 2, os.path.join(benign_savedir, 'arith_org_inputs_%d.jpg' % batch_idx))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_existing_data', type=bool, default=False, help='use hybrid adversarial examples from existing dirs')
    parser.add_argument('--max_num_fusion', type=int, default=150, help='the times of fusion')
    ...
    all_dataset = ['ffhq']
    adversarial_choose = ['white_box_target']
    ...
    for dataset in all_dataset:
        ...
        for adversarial in adversarial_choose:
            ...
            # attacks = PGD(model, eps=8 / 255, alpha=0.01, steps=100, random_start=False)
            # labels = [0, 0, 1, 1, 1]
            ...
            for batch_idx, batch in enumerate(test_dataloader):  # dataset_n_dict[args.dataset_name] as a batch
                ...
                adv_inputs = main_optimize(inputs.clone(), drawer, net, target_img, args, device, iter_dict, train_dataloader, adv_savedir)
                # adv_inputs = attacks(inputs, labels)
                ...
                all_adv_latents = get_latents(net,
                                              F.avg_pool2d(adv_inputs, int(net.decoder.size / 256),
                                                           int(net.decoder.size / 256)),
                                              is_cars)
                partial_adv_fusion_spati = partial_adv_fusion_spatial(adv_savedir, inputs, adv_inputs, all_latents, all_adv_latents)
                partial_adv_fusion_arith = partial_adv_fusion_arithmetic(adv_savedir, inputs, adv_inputs, all_latents, all_adv_latents)

                # print(cal_SSMI(I_fused.squeeze(0).cpu(), adv_I_fused.squeeze(0).cpu()))

                criterion = nn.MSELoss(reduction='none').to(device)
                noise = torch.mean(criterion(inputs, adv_inputs), dim=(1, 2, 3))
